...
import { ChevronLeftIcon, ChevronRightIcon, LayoutDashboardIcon, Link2Icon, MenuIcon, MessageSquareIcon, PaletteIcon } from 'lucide-react';
...
const menuItems = [
	...
	{ path: '/feedback', icon: MessageSquareIcon, text: 'Feedback' },
	{ path: '/appearance', icon: PaletteIcon, text: 'Appearance' },
	{ path: '/integrations', icon: Link2Icon, text: 'Integrations' }
];
					...
					<ul className='space-y-2 font-medium flex-grow'>
						{menuItems.map((item, idx) => (
							<Link
								...
								onClick={closeMenu}
								className={cn(
									'flex items-center px-6 py-3 border-r-4 border-transparent text-black transition-colors duration-150 dark:text-white hover:bg-blue-600 hover:text-white group',
						...
						<div className={cn('flex items-center justify-between p-4 gap-x-4', isCollapsed && 'justify-center')}>
							<div className={cn('flex flex-col gap-y-1 flex-shrink-0', isCollapsed && 'lg:hidden')}>
								...
								<span className='text-xs text-muted-foreground whitespace-nowrap'>
									Customize how FireFeed looks <br /> on your device
								</span>
							</div>
							<ModeToggle />
						</div>
